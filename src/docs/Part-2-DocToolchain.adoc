...
Über Asciidoctor als Kommandozeilen-Werkzeug oder Plugin für Gradle bzw. Maven generieren Sie anschließend HTML (Schritt 2).
...
PlantUML scheitert an der Modellierung umfangreicher Diagramme, mangels geeigneter Möglichkeiten, das Layout nach Ihren Vorstellungen Einfluss zu beeinflussen.
...
Wenn Sie z.B. den Namen einer Komponente in Ihrer Modellierung ändern, wird es schwierig nur die Diagramme erneut zu exportieren, die sich geändert haben: Oft ist unklar, welche Diagramme von dieser Namensänderung betroffen sind. Somit müssen Sie regelmäßig _alle_ Diagramme aktualisieren.
...
Für gängige UML-Tools ist die Wahrscheinlichkeit hoch, dass ein solcher automatischer Export funktioniert!
...
Der EA verfügt über eine COM-Schnittstelle, so dass Sie ihn von außerhalb der Anwendung fernsteuern können.
...
Der Task https://doctoolchain.github.io/docToolchain/#_exportexcel[`exportExcel`] sucht nach allen Excel-Dateien im Projekt, öffnet sie und konvertiert den Inhalt Arbeitsblatt für Arbeitsblatt nach AsciiDoc.
Das Ergebnis inkludieren Sie einfach wieder in Ihre Dokumentation.
...
Bezüglich der exportierten Texte und Grafiken weichen wir vom Schema des Softwareentwicklers ab und nehmen auch diese generierten Artefakte unter Versionskontrolle.
...
Aber dafür haben Sie die Diagramme ja unter Versionskontrolle...
...
Anschließend sollten Sie den Asciidoctor-Task duplizieren.
...
Mit `.yaml`-basierten Styles können Sie anschließend noch jedes Detail des PDFs konfigurieren.
...
wie z.B. unterschiedliche Fußzeilen für linke und rechte Seite.
...
Dies nutzen wir und wandeln mit Hilfe von https://pandoc.org/[Pandoc] (dem Schweizer Taschenmesser der Formatkonvertierung) anschließend das docBook-Format weiter in DOCX oder auch ePUB.
